...
const setup = async package => {
	...
	if ( package.wporg.type === 'plugin' ) {
		...
		package.wporg.wporg = { svn, ...response }
		...
		console.log( `\`wp-release\` will create package zipfile`.cyan );
		console.log( `you will have to upload it manually`.cyan );
	}
...
const svn = ( dry = false ) => {
	...
	current_task = doing('Gathering data');
	current_task.say('create dirs')
