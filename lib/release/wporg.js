	...
	if ( package.wporg.type === 'plugin' ) {
		...
		const response = await prompts([
			...
			},
			{
				type: 'text',
				name: 'svn_user',
				message: `WP.org Username (Leave blank use existing svn credentials):`,
				initial: ''
			}
		], { onCancel: process.exit });
		if ( ! response.svn_user ) {
			delete( response.svn_user )
		}
...
const svn = ( dry = false ) => {
	...
	if ( ! dry ) {
		...
		if ( package.wporg.wporg.svn_user ) {
			ci_cmd = `svn commit -m "${commit_message}" --username ${package.wporg.wporg.svn_user}`
		} else {
